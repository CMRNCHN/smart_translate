...
function toConstExport(source, constName) {
  ...
  const exportMatch = out.match(/module\.exports\s*=\s*(\{[\s\S]*\});?\s*$/);
  if (!exportMatch) {
    throw new Error(`No module.exports found for ${constName}`);
  }
  const exportBody = exportMatch[1];
  out = out.replace(/module\.exports\s*=\s*\{[\s\S]*\};?\s*$/, "").trim();
